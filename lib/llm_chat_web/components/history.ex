  ...
  def history(assigns) do
    ~H"""
      <div class="flex flex-col w-1/5 h-screen bg-gray-200">
        <div class="h-1/2 flex-grow p-4 space-y-4 overflow-y-auto">
          ...
          <div class="history">
            <%= for {id, description} <- @conversations do %>
              ...
              <%= if @id == id do %>
                <ul class="cursor-pointer bg-blue-50 rounded-xl hover:bg-gray-50 hover:rounded-xl border-b border-gray-300 py-2 truncate text-ellipsis" phx-click="load_conversation" phx-value-id={id}>
